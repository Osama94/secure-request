...
/// guarantees, use AWS KMS directly with RSA key pairs and perform decrypt operations
...
public sealed class AwsSecretsManagerKeyStorageProvider : IRsaKeyStorageProvider
{
    ...
    public async Task<byte[]?> LoadPrivateKeyAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogDebug(
                "[SecureRequest.AwsSecretsManager] Loading RSA private key from secret '{SecretId}'.",
                _secretId);
            ...
            _logger.LogInformation(
                "[SecureRequest.AwsSecretsManager] RSA private key loaded from secret '{SecretId}'.",
                _secretId);
        ...
        {
            _logger.LogInformation(
                "[SecureRequest.AwsSecretsManager] Secret '{SecretId}' not found — a new RSA key pair will be generated.",
                _secretId);
            return null;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex,
                "[SecureRequest.AwsSecretsManager] Failed to load RSA private key from secret '{SecretId}'.",
                _secretId);
            throw;
        }
    }
    ...
    public async Task StorePrivateKeyAsync(byte[] privateKeyBytes, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            _logger.LogInformation(
                "[SecureRequest.AwsSecretsManager] RSA private key updated in secret '{SecretId}'.",
                _secretId);
        }
        catch (ResourceNotFoundException)
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    "[SecureRequest.AwsSecretsManager] RSA private key created in secret '{SecretId}'.",
                    _secretId);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex,
                    "[SecureRequest.AwsSecretsManager] Failed to create secret '{SecretId}'.",
                    _secretId);
                throw;
            }
        }
        ...
        {
            _logger.LogError(ex,
                "[SecureRequest.AwsSecretsManager] Failed to store RSA private key in secret '{SecretId}'.",
                _secretId);
            throw;
        }
    }
