...
dotnet add package SecureRequest.AwsKms
```
...
That's it. The AWS SDK default credential chain is used automatically — it picks up IAM roles (EC2/ECS/Lambda), environment variables, and `~/.aws/credentials` in development.
...
The IAM role / user running the application needs:
...
## Compared to the default provider

| | `DistributedCacheKeyStorageProvider` | `AwsSecretsManagerKeyStorageProvider` |
|---|---|---|
