...
## Companion packages — Cloud KMS

Plug-and-play companions replace the default Redis key storage with a dedicated cloud KMS in one line. All companions require `SecureRequest` v1.2.0+.

| Package | Cloud | One-liner |
|---------|-------|-----------|
| [`SecureRequest.AzureKeyVault`](https://www.nuget.org/packages/SecureRequest.AzureKeyVault) | Azure Key Vault | `.WithAzureKeyVault(uri)` |
| [`SecureRequest.AwsSecretsManager`](https://www.nuget.org/packages/SecureRequest.AwsSecretsManager) | AWS Secrets Manager | `.WithAwsSecretsManager()` |
| [`SecureRequest.GcpSecretManager`](https://www.nuget.org/packages/SecureRequest.GcpSecretManager) | GCP Secret Manager | `.WithGcpSecretManager(projectId)` |
...
```csharp
// Secrets mode — private key stored as a Key Vault secret
builder.Services
    .AddSecureRequest(builder.Configuration)
    .WithAzureKeyVault("https://your-vault.vault.azure.net/");

// True HSM mode — private key NEVER leaves Key Vault (zero-export)
builder.Services
    .AddSecureRequest(builder.Configuration)
...
`DefaultAzureCredential` is used automatically (Managed Identity, Azure CLI, Visual Studio).
...
builder.Services
    .AddSecureRequest(builder.Configuration)
    .WithGcpSecretManager(projectId: "my-gcp-project"); // uses Application Default Credentials
```

### Custom provider (bring your own)

Implement `IRsaKeyStorageProvider` and register it with `WithKeyStorage<T>()`:

```csharp
builder.Services
    .AddSecureRequest(builder.Configuration)
    .WithKeyStorage<MyHashiCorpVaultProvider>();

// Or use the factory overload when constructor args aren't in DI:
builder.Services
    .AddSecureRequest(builder.Configuration)
...
### Local / single-instance (no Redis, no cloud KMS)

```csharp
builder.Services
    .AddSecureRequest(builder.Configuration)
    .WithMemoryStorage(); // key held in process memory — good for dev and single-instance apps
```
...
const { encryptedBody, headers } = await buildSecureRequest('POST', '/api/users', '', { name: 'Osama Ahmed' });
